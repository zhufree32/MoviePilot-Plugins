...
class DownloadSiteTag(_PluginBase):
    ...
    # 插件描述
    plugin_desc = "自动给下载任务分类与打站点标签、剧集名称标签（支持手动添加的任务）"
    # 插件图标
    ...
    def _parse_media_type_from_name(self, torrent_name: str) -> Optional[str]:
        """
        从种子名称解析媒体类型
        返回: Movie/TV/Anime 或 None
        """
        if not torrent_name:
            return None
        ...
        # 动漫关键词
        anime_keywords = ["动漫", "动画", "anime", "cartoon", "二次元", "番剧", "動漫", "アニメ"]
        if any(keyword in name for keyword in anime_keywords):
            return MediaType.TV.value  # 动漫归类到TV类型，后续会通过genre_ids判断
        
        # 电视剧特征：SxxExx 格式 (S01E01, S1E1, 1x01 等)
        tv_patterns = [
        ...
        if any(re.search(pattern, name) for pattern in tv_patterns):
        ...
        for service in self.service_infos.values():
            ...
            for torrent in torrents:
                try:
                    ...
                    # ========== 核心修改1：处理无下载历史的种子 ==========
                    manual_added = False
                    torrent_name = torrent.get("name") if service.type == "qbittorrent" else torrent.name
                    site_name = None
                    media_type = None
                    media_title = None
                    ...
                    if not history:
                        ...
                        for tracker in trackers:
                            ...
                            if site_info:
                                ...
                                history.torrent_site = site_name
                                break
                        
                        # 手动添加的种子：2. 从文件名解析媒体类型
                        media_type = self._parse_media_type_from_name(torrent_name)
                        history.type = media_type
                        
                        # 手动添加的种子：3. 提取媒体标题（简单处理，取第一个括号前的内容）
                        if torrent_name:
                            # 移除分辨率、编码等后缀，提取主标题
                            title_match = re.match(r'^([^.(\[]+)', torrent_name)
                            if title_match:
                                media_title = title_match.group(1).strip()
                            else:
                                media_title = torrent_name[:50]  # 防止标题过长
                            history.title = media_title
                    else:
                        # 有下载历史的种子，使用原有逻辑
                        site_name = history.torrent_site
                        media_type = history.type
                        media_title = history.title
                        # 加入历史记录
                        if _key:
                    ...
                    if indexers.intersection(set(torrent_tags)):
                        history.torrent_site = None
                    # 如果站点名称为空, 尝试通过trackers识别（原有逻辑保留）
                        ...
                        for tracker in trackers:
                            ...
                            if site_info:
                                ...
                                site_name = history.torrent_site  # 更新site_name
                                break
                        # ========== 核心修改2：不再跳过无历史的种子 ==========
                        # 移除原有跳过逻辑，即使没有站点和媒体信息也继续（至少尝试处理）
                    ...
                    # 站点标签, 如果勾选开关的话
                    if self._enabled_tag and site_name:
                        site_tag = self._generate_site_tag(site_name)
                    ...
                    # 分类, 如果勾选开关的话 <tr暂不支持>
                    if service.type == "qbittorrent" and self._enabled_category and not torrent_cat and media_type:
                        # 如果是电视剧 需要区分是否动漫
                        genre_ids = None
                        # 手动添加的种子：尝试判断是否为动漫
                        if manual_added and media_type == MediaType.TV.value:
                            # 从名称判断是否为动漫
                            if any(keyword in torrent_name.lower() for keyword in ["动漫", "动画", "anime", "アニメ"]):
                                genre_ids = settings.ANIME_GENREIDS
                        else:
                            # 有下载历史的种子：原有逻辑
                        ...
                        _cat = self._genre_ids_get_cat(media_type, genre_ids)
                    ...
                    if torrent_tags:
                        ...
                        if site_name:
                            tags_to_remove.extend(
                                self._get_tags_to_remove_for_prefix(site_name, self._site_prefix, torrent_tags)
                            )
                        
                        # 剧名标签处理
                        if media_title:
                            tags_to_remove.extend(
                                self._get_tags_to_remove_for_prefix(media_title, self._media_prefix, torrent_tags)
                            )
    ...
    def _genre_ids_get_cat(self, mtype, genre_ids=None):
        ...
        if mtype == MediaType.MOVIE or mtype == MediaType.MOVIE.value:
            ...
            if genre_ids and set(genre_ids).intersection(set(ANIME_GENREIDS)):
                # 动漫
