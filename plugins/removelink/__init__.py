...
class RemoveLink(_PluginBase):
    ...
    plugin_desc = "监控STRM/字幕文件删除，同步删除目标目录对应文件+空目录（STRM→同名视频，字幕→完全同名文件）支持CloudDrive2等多种存储"
    plugin_icon = "Ombi_A.png"
    plugin_version = "3.0"
    plugin_author = "zhufree（增强版）"
    ...
    def init_plugin(self, config: dict = None):
        logger.info(f"初始化STRM&字幕文件清理插件（支持多存储）")
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "type": "info",
                                            "variant": "tonal",
                                            "title": "🧹 STRM&字幕文件清理插件（支持多存储）",
                                            "text": "1. 监控STRM删除 → 同步删除目标目录「同名（去后缀）」视频文件；2. 监控SRT/ASS删除 → 同步删除目标目录「完全同名（含后缀）」字幕文件；3. 删除文件后自动清理空目录（逐级向上）；4. 支持本地、CloudDrive2、阿里云盘、115网盘等多种存储系统。",
                                        },
                                    }
                                ],
                            },
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            "type": "warning",
                                            "variant": "tonal",
                                            "text": "注意：请确保对应的存储插件已正确配置并启用，否则删除操作会失败。",
                                        },
                                    }
                                ],
                            },
    ...
    def _parse_strm_path_mappings(self) -> Dict[str, Tuple[str, str]]:
        ...
        for line in self.strm_path_mappings.split("\n"):
            ...
            try:
                ...
                logger.debug(f"解析路径映射：{local_path} -> [{storage_type}] {storage_path}")
            except Exception as e:
                logger.warning(f"解析路径映射失败：{line}，错误：{e}")
        return mappings

    def _get_available_storages(self) -> List[str]:
        """获取所有可用的存储类型"""
        try:
            # 通过StorageChain获取可用的存储类型
            # 这里假设StorageChain有方法获取所有存储类型
            # 如果没有，我们可以尝试获取系统配置中的存储类型
            storages = []
            # 尝试从系统配置获取
    ...
    def _get_target_path(self, local_file_path: Path, keep_suffix: bool = True) -> Tuple[Optional[str], Optional[str]]:
        ...
        logger.warning(f"本地文件 {local_file_path} 未找到对应的路径映射")
        return None, None
        ...
        try:
            ...
            while current_dir_path not in root_markers:
                ...
                try:
                    ...
                    logger.error(f"检查目录存在性失败 [{storage_type}] {current_dir_path}: {e}")
                    break
                ...
                try:
                    ...
                    logger.error(f"列出目录内容失败 [{storage_type}] {current_dir_path}: {e}")
                    break
